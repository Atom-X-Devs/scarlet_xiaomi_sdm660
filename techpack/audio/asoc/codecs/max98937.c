...
static int max98927_dai_set_fmt(struct snd_soc_dai *component_dai, unsigned int fmt)
{
...
static int max98927_stream_mute(struct snd_soc_dai *component_dai, int mute,
				int stream)
{
	struct snd_soc_component *component = component_dai->component;
	struct max989xx_priv *max98927 = snd_soc_component_get_drvdata(component);
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (mute) {
			...
			if (max98927->mono_stereo == 0x0 || (max98927->mono_stereo & 0x1)) {
				if (!rdc_check_valid(max98927->ref_RDC[MAX98927L], MAX98927L) &&
				    max98927->ref_RDC[MAX98927L] != SPK_MUTE_VALUE) {
					rc = max989xx_calib_get(&impedance, MAX98927L);
					if (rdc_check_valid(impedance, MAX98927L) ||
			...
			if (max98927->mono_stereo & 0x2) {
				if (!rdc_check_valid(max98927->ref_RDC[MAX98927R], MAX98927R) &&
				    max98927->ref_RDC[MAX98927R] != SPK_MUTE_VALUE) {
					impedance = 0;
					rc = max989xx_calib_get(&impedance, MAX98927R);
					if (rdc_check_valid(impedance, MAX98927R) ||
					    impedance == SPK_MUTE_VALUE) {
	...
	if (!max98927->bIsMax98937[MAX98927L]) {
		if (mix) {
			...
			for (i = 0; i < ARRAY_SIZE(max98927_reg_channel_map[MAX98927L + 1]); i++)
				regmap_write(max98927->regmap[MAX98927L],
					     max98927_reg_channel_map[MAX98927L + 1][i][0],
		...
		if (mix) {
			...
			for (i = 0; i < ARRAY_SIZE(max98937_reg_channel_map[MAX98927L + 1]); i++)
				regmap_write(
					max98927->regmap[MAX98927L],
					max98937_reg_channel_map[MAX98927L + 1][i][0],
...
{
	struct max989xx_priv *max98927 = snd_soc_component_get_drvdata(component);
	/* struct snd_soc_dapm_context *dapm = &ccomponent->dapm;  */
