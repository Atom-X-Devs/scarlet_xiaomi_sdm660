...
static int elliptic_store_data(const char *buffer, size_t buffer_size, )
{
	int ret = 0;
	int id = 0;
	...
	case ELLIPTIC_ULTRASOUND_PARAM_ID_ENGINE_VERSION:
		if (buffer_size >= ELLIPTIC_VERSION_INFO_SIZE) {
			pr_debug(
				"[ELUS]: elliptic_version copied to local AP cache");
			...
			memcpy((u8 *)data_block->buffer, &payload[3],
			       copy_size);
...
void elliptic_data_io_ipi_handler(int id, void *data, unsigned int len)
{
	...
	elliptic_data_push(ELLIPTIC_ALL_DEVICES, (const char *)data, max_len,
...
int elliptic_data_io_initialize(void)
{
	...
	ipi_registration_result = scp_ipi_registration(
		IPI_USND, elliptic_data_io_ipi_handler, "usnd");
			       ...
			       size_t data_size)
{
	...
	send_status =
		scp_ipi_send(IPI_USND, &host_message, sizeof(host_message), 1);
