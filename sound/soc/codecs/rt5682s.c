...
static int rt5682s_button_detect(struct snd_soc_component *component)
{
	...
	val = snd_soc_component_read32(component, RT5682S_4BTN_IL_CMD_1);
	btn_type = val & 0xfff0;
...
static int rt5682s_headset_detect(struct snd_soc_component *component, int jack_insert)
{
	...
	if (jack_insert) {
		...
		do {
			...
			val = snd_soc_component_read32(component, RT5682S_CBJ_CTRL_2)
...
static void rt5682s_jack_detect_handler(struct work_struct *work)
{
	...
	val = snd_soc_component_read32(rt5682s->component, RT5682S_AJD1_CTRL)
		& RT5682S_JDH_RS_MASK;
	if (!val) {
		/* jack in */
...
static void rt5682s_jd_check_handler(struct work_struct *work)
{
	...
	if (snd_soc_component_read32(rt5682s->component, RT5682S_AJD1_CTRL)
		& RT5682S_JDH_RS_MASK) {
		/* jack out */
		rt5682s->jack_type = rt5682s_headset_detect(rt5682s->component, 0);
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	val = snd_soc_component_read32(component, RT5682S_GPIO_CTRL_1)
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		val = (snd_soc_component_read32(component, RT5682S_GLB_CLK)
		...
		struct snd_soc_dapm_widget *sink)
{
	...
	val = (snd_soc_component_read32(component, reg) >> sft) & 0xf;
	switch (val) {
	case RT5682S_CLK_SEL_I2S1_ASRC:
	case RT5682S_CLK_SEL_I2S2_ASRC:
		return 1;
					     ...
					     unsigned long parent_rate)
{
	...
	bclks_per_wclk = snd_soc_component_read32(component, RT5682S_TDM_TCON_CTRL_1);
	...
	for (i = 0; i < RT5682S_DAI_NUM_CLKS; ++i) {
		...
		case RT5682S_DAI_BCLK_IDX:
			...
			parent_name = __clk_get_name(
				rt5682s->dai_clks[RT5682S_DAI_WCLK_IDX]);
			init.parent_names = &parent_name;
			init.num_parents = 1;
			break;
		default:
		...
		if (dev->of_node) {
			...
			if (!dai_clk_lookup) {
				...
				rt5682s->dai_clks_lookup[i] = dai_clk_lookup;
			}
		}
	}
