...
static inline void update_misfit_status(struct task_struct *p, struct rq *rq)
{
	...
	cpu = cpu_of(rq);
	if (task_fits_capacity(p, capacity_orig_of(cpu), cpu)) {
		rq->misfit_task_load = 0;
		return;
	}
...
select_idle_capacity(struct task_struct *p, struct sched_domain *sd, int target)
{
	...
	for_each_cpu_wrap(cpu, cpus, target) {
		...
		if (!available_idle_cpu(cpu) && !sched_idle_cpu(cpu))
			continue;
		if (fits_capacity(task_util, cpu_cap, cpu))
			return cpu;
...
static int select_idle_sibling(struct task_struct *p, int prev, int target)
{
	...
	if (prev != target && cpus_share_cache(prev, target) &&
	    (available_idle_cpu(prev) || sched_idle_cpu(prev)) && !cpu_isolated(prev) &&
				     ...
				     int sibling_count_hint)
{
	...
	if (fbt_env.need_idle || need_idle || boosted ||
		    ...
		    int sibling_count_hint)
{
	...
	if (sd_flag & SD_BALANCE_WAKE) {
		...
		if (sysctl_sched_sync_hint_enable && sync && _cpus_allowed &&
		    cpu_rq(cpu)->nr_running == 1 && cpu_is_in_target_set(p, cpu)) {
			return cpu;
		}
