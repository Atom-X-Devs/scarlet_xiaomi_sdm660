...
/*
 * Tracepoint for debugging sched event of EAS runtime.
 */
TRACE_EVENT(sched_debug_einfo,
		TP_PROTO(struct task_struct *tsk, const char *flag1,
			 const char *flag2, unsigned int param1,
			 unsigned int param2, u64 se_vr, u64 en_vr,
			 u64 cfq_min_vr),
		TP_ARGS(tsk,flag1, flag2, param1, param2, se_vr, en_vr,
			cfq_min_vr),
		...
		TP_printk("comm=%s pid=%d, %s=%d %s=%d, sv=%Lu [ns] ev=%Lu [ns] cv=%Lu [ns]",
			__entry->comm,
			__entry->pid,
			__entry->flag1,
			__entry->param1,
