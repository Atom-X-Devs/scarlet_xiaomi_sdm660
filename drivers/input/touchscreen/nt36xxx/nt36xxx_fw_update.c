...
int32_t Check_FW_Ver(void)
{
	...
	return (buf[1] > fw_entry->data[FW_BIN_VER_OFFSET]) ? 1 : 0;
}
...
int32_t Check_CheckSum(void)
{
	...
	for (i = 0; i < BLOCK_64KB_NUM; i++) {
		if (fw_need_write_size > (i * SIZE_64KB)) {
			len_in_blk = min(fw_need_write_size - i * SIZE_64KB,
					 (size_t)SIZE_64KB);
	...
	for (i = 0; i < count; i++) {
		...
		if (fw_need_write_size - Flash_Address >= 256)
			...
			tmpvalue = (Flash_Address >> 16) +
				   ...
				   (fw_need_write_size - Flash_Address - 1);
