							...
							unsigned int link_id)
{
	...
	if (he_cap->has_he) {
		...
		if (eht_cap->has_eht &&
		    nl80211_is_6ghz(link_conf->chandef.chan->band)) {
...
void ieee80211_sta_set_rx_nss(struct sta_info *sta, unsigned int link_id)
{
	...
	if (cfg_eht_cap_has_eht(sta->sta.link[link_id])) {
		int i;
		const u8 *rx_nss_mcs = (void *)&cfg_eht_cap(sta->sta.link[link_id])->eht_mcs_nss_supp;
