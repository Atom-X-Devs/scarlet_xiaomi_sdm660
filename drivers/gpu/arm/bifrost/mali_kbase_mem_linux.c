...
#ifdef CONFIG_MALI_BIFROST_MEMORY_FULLY_BACKED
...
int kbase_mem_flags_change(struct kbase_context *kctx, u64 gpu_addr, unsigned int flags, unsigned int mask)
{
	...
	if (IS_ENABLED(CONFIG_MALI_BIFROST_DMA_BUF_MAP_ON_DEMAND)) {
		...
		/*
		 * When CONFIG_MALI_BIFROST_DMA_BUF_MAP_ON_DEMAND is not enabled the
		...
		struct kbase_va_region *reg, enum kbase_sync_type sync_fn)
{
	...
	if (reg->gpu_alloc->type != KBASE_MEM_TYPE_IMPORTED_UMM)
		return ret;
	/*
	 * Attempting to sync with CONFIG_MALI_BIFROST_DMA_BUF_MAP_ON_DEMAND
		...
		struct kbase_va_region *reg)
{
	...
	if (alloc->imported.umm.current_mapping_usage_count != 1) {
		if (IS_ENABLED(CONFIG_MALI_BIFROST_DMA_BUF_LEGACY_COMPAT) ||
